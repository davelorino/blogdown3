...
```
Call str() on the dataset and take a look and what we’ve got:
```{r seats_data}
str(seats_data[,1:12])
```
...
leaflet() %>%
  addTiles
```
Looks good, but it’s not really focused on Tasmania - when we plot our seats we’ll have to navigate manually in the map in order to find them - not ideal. Let’s make it so the map looks directly at Launceston, Tasmania from the get-go.
```{r}
leaflet() %>%
  addTiles() %>%
